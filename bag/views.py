    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]["items_by_size"].keys():
                ...
                messages.success(
                    request,
                    f'No worries we updated {size.upper()} {product.name} \
                ...
                messages.success(
                    request,
                    f"Hooray we added size {size.upper()} {product.name} \
                        to your bag!",
                )
        else:
            bag[item_id] = {"items_by_size": {size: quantity}}
            messages.success(request, f"Hooray we added size {size.upper()} \
                {product.name} to your bag!")
    
    else:
        if item_id in list(bag.keys()):
            bag[item_id] += quantity
            messages.success(request, f"No worries we updated {product.name} \
                quantity to {bag[item_id]}!")
        else:
            bag[item_id] = quantity
            messages.success(request, f'Hooray we \
